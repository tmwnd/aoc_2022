...
def part_one():
    print("--- part one ---")
    print(sum([(i+1) * int((compare_signals(data[2*i], data[2*i+1]) <= 0))
...
def part_two():
    ...
    data.append([[2]])
    data.append([[6]])
    print(sum([int(compare_signals(signal, [[2]]) <= 0) for signal in data])
          * sum([int(compare_signals(signal, [[6]]) <= 0) for signal in data]))
